...
class Checkout
  ...
  def scan(item_code)
    raise KeyError.new, "Item not found in inventory" unless @pricing_rules.key?(item_code) && item_code != "price"
    @counter[item_code] += 1
  end
  ...
  def validate(pricing_rules)
    raise ArgumentError.new, "pricing_rules argument missing" if pricing_rules.nil?
